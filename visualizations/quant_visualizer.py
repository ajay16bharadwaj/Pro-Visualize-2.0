...
class QuantificationVisualizer:
    ...
    def plot_venn_diagram(self, selected_groups: list, **kwargs):
    ...
    def plot_upset(self, **kwargs):
        ...
        for protein in all_proteins:
            protein_membership = [group for group, proteins in protein_sets.items() if protein in proteins]
        ...
        title = kwargs.get('title', "Protein Intersections Across Experimental Groups")
        fig.suptitle(title, fontsize=12)
        return fig
    
    def plot_protein_counts(self, color_map=None, **kwargs):
        ...
        plot_args = {
            ...
            "title": kwargs.get('title', "Quantified Proteins per Sample"),
            "labels": {'Sample': 'Sample', 'ProteinCount': 'Number of Proteins'}
        }

        if self.annotation_df is not None:
            plot_data = pd.merge(protein_counts, self.annotation_df, left_on='Sample', right_on=self.sample_col, how='left')
            ...
            if color_map: plot_args["color_discrete_map"] = color_map
        else:
        ...
        fig.update_layout(
            xaxis_tickangle=-45, height=kwargs.get('height', 500),
            width=800, margin=dict(b=120),
            template=kwargs.get('template', 'plotly_white')
        )
        return fig
    
    def plot_missing_values_heatmap(self, color_map=None, **kwargs):
        if self.annotation_df is None:
            raise ValueError("Annotation data is required to group samples.")
        ...
        if df_missing.empty:
            return go.Figure().update_layout(title="No Missing Proteins Found")
        ...
        palette = [color_map.get(g, default_colors[i % len(default_colors)]) for i, g in enumerate(group_names)] if color_map else default_colors
        ...
        n = len(group_names)
        if n > 1:
            for i in range(n): current_palette_list.append([i / (n - 1), palette[i % len(palette)]])
        else: current_palette_list = [[0, palette[0]], [1, palette[0]]]

        fig.add_trace(go.Heatmap(
            z=[group_colors_numeric.values], x=sorted_samples, y=['Group'],
            colorscale=current_palette_list, showscale=False
        ), row=2, col=1)
        ...
        for i, group in enumerate(group_names):
            ...
            fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers', name=group, marker=dict(size=10, color=c, symbol='square')))

        fig.update_layout(
            title_text=kwargs.get('title', 'Missing Proteins Pattern'), height=kwargs.get('height', 600),
            xaxis_tickangle=-90, margin=dict(b=150, l=80), legend_title_text='Legend',
            template=kwargs.get('template', 'plotly_white')
        )
        return fig
    
    def plot_missing_value_distribution(self, **kwargs):
        df_long = self.protein_df.melt(id_vars=[self.protein_col], value_vars=self.sample_cols, var_name='Sample', value_name='Intensity')
        ...
        if dist_data_missing.empty or dist_data_observed.empty:
            return go.Figure().update_layout(title="Cannot Generate Plot: Missing data required")
        ...
        fig.update_layout(
            title_text=kwargs.get('title', 'Missing Value Distribution'),
            height=kwargs.get('height', 500), showlegend=True,
            template=kwargs.get('template', 'plotly_white')
        )
    ...
    def plot_protein_overlap(self, **kwargs):
        ...
        overlap_counts = protein_presence.value_counts().sort_index().reset_index()
        overlap_counts.columns = ['Num_Samples', 'Protein_Count']
        fig = px.bar(overlap_counts, x='Num_Samples', y='Protein_Count', title=kwargs.get('title', "Protein Overlap"))
        fig.update_layout(
            xaxis_type='category', height=kwargs.get('height', 500), width=600,
            template=kwargs.get('template', 'plotly_white')
        )
        return fig
    ...
    def plot_protein_coverage_chart(self, **kwargs):
        ...
        fig = px.bar(plot_data, x='category', y='value', title=kwargs.get('title', "Protein Coverage"))
        fig.update_layout(
            showlegend=False, yaxis_title="Number of proteins", xaxis_showticklabels=True,
            uniformtext_minsize=8, uniformtext_mode='hide', height=kwargs.get('height', 500),
            template=kwargs.get('template', 'plotly_white')
        )
        fig.for_each_trace(lambda t: t.update(marker_color='#333333', marker_line_color='white', marker_line_width=0.5))
        fig.update_yaxes(range=[0, total_proteins * 1.05])
        fig.update_xaxes(linecolor='black', showline=True)
        return fig
    
    def plot_intensity_distribution(self, color_map=None, **kwargs):
        if self.annotation_df is None:
        ...
        palette = None
        if color_map:
            sample_to_group = dict(zip(self.annotation_df[self.sample_col], self.annotation_df[self.group_col]))
            palette = {sample: color_map.get(group, '#333333') for sample, group in sample_to_group.items()}

        g = sns.FacetGrid(merged_data, col=self.group_col, hue='Sample', col_wrap=3, height=4, aspect=1.2, sharex=True, sharey=True, palette=palette)
        ...
        g.fig.suptitle(kwargs.get('title', "Protein Intensity Density Distribution"), y=1.03, fontsize=16)
    ...
    def get_transcription_factor_count(self) -> int:
        if 'Gene Name' not in self.protein_df.columns: return 0
        return self.protein_df['Gene Name'].isin(self.HUMAN_TRANSCRIPTION_FACTORS).sum()   

    def plot_protein_rank_order(self, proteins_to_annotate=None, color_by_option='None',
                                custom_list=None, keyword=None, annotate_highlighted=False, **kwargs):
        ...
        rank_df = pd.merge(rank_df, self.protein_df[[self.protein_col] + protein_info_cols].drop_duplicates(), on=self.protein_col, how='left')
        ...
        fig = px.scatter(
            ...
            title=kwargs.get('title', "Protein Rank Order by Average Intensity")
        )
        ...
        fig.update_layout(
            ...
            legend_title_text="Category", height=kwargs.get('height', 600),
            template=kwargs.get('template', 'plotly_white')
        )
        if 'marker_size' in kwargs: fig.update_traces(marker=dict(size=kwargs['marker_size']))
        return fig
    ...
    def plot_pca_by_annotation(self, color_by: str, symbol_by: str = None, 
        ...
        discrete_map = color_map if (color_map and color_by == self.group_col) else None

        fig = px.scatter(
            plot_df, x=pc_x_label, y=pc_y_label, color=color_by, symbol=symbol_by,
            title=kwargs.get('title', f"PCA Colored by '{color_by}'"),
            hover_data=[self.sample_col], text=plot_df[self.sample_col] if show_labels else None,
            color_discrete_map=discrete_map
        )
        fig.update_traces(marker=dict(size=kwargs.get('marker_size', 10)), textposition='top center')
        fig.update_layout(height=kwargs.get('height', 600), template=kwargs.get('template', 'plotly_white'))
        return fig
    ...
    def plot_pca_with_clusters(self, n_clusters: int = 3, show_labels: bool = False, **kwargs):
        ...
        for cluster_id in range(n_clusters):
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(color=color, size=kwargs.get('marker_size', 10)),
                name=f'Cluster {cluster_id}', text=cluster_points['Sample'],
            ...
            if len(cluster_points) > 2:
                ...
                fig.add_trace(go.Scatter(
                    x=cluster_points['PC1'].iloc[hull_points], y=cluster_points['PC2'].iloc[hull_points],
                    fill="toself", fillcolor=color, opacity=0.2, line=dict(color=color, width=1), showlegend=False
                ))
        
        fig.update_layout(
            title=kwargs.get('title', f'PCA with {n_clusters} Clusters'),
    ...
    def plot_dendrogram(self, method='ward', **kwargs):
        scaled_data, samples = self._prepare_data_for_clustering()
        ...
        plt.title(kwargs.get('title', 'Hierarchical Clustering Dendrogram'), fontsize=16)
        plt.xlabel('Distance')
        plt.tight_layout()
        buf = BytesIO()
